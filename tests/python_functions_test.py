...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
    cube_volume = length_for_volume(5)
    self.assertEqual(125, cube_volume)

  #Given a String, return the String reversed
  def test_reverse_string(self):
    string = "Hello World"
    reversed_string = string_to_reverse(string)
    self.assertEqual("dlroW olleH", reversed_string )

  #Given a value in farenheit, convert this into celsius.
  def test_fahrenheit_to_celsius(self):
    fahrenheit_temp = 54
    celcius_temp = convert_to_celcius(fahrenheit_temp)
    self.assertEqual(12, celcius_temp)
